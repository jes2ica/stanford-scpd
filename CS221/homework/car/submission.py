...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                ...
                self.belief.setProb(row, col, prob * util.pdf(mean, Const.SONAR_STD, observedDist))
    ...
    def elapseTime(self):
        ...
        sum = self.belief.getSum()
        prob = {}
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                prob[(row, col)] = self.belief.getProb(row, col)
                self.belief.setProb(row, col, 0)

        for key, value in self.transProb.items():
            oldTile = key[0]
            newTile = key[1]
            self.belief.addProb(newTile[0], newTile[1], value * prob[oldTile])
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for tile in self.particles:
            ...
            weightDict[tile] = pdf * count

        self.particles = collections.Counter()
        for i in range(self.NUM_PARTICLES):
            newParticle = util.weightedRandomChoice(weightDict)
