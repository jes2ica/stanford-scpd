...
def create_nqueens_csp(n = 8):
    ...
    for i1, var1 in enumerate(variables):
        for i2, var2 in enumerate(variables):
            ...
            csp.add_binary_factor(var1, var2, lambda x, y : (x[0] + x[1]) != (y[0] + y[1]))
            csp.add_binary_factor(var1, var2, lambda x, y : (x[0] - x[1]) != (y[0] - y[1]))
    return csp
                ...
                for val2 in self.domains[var2]:
                    ...
                    for val1 in self.domains[var1]:
                        if self.csp.binaryFactors[var1][var2][val1][val2] > 0:
                            canRemove = False
                    if canRemove:
                        toRemove.append(val2)
                if len(toRemove) > 0:
                    for val in toRemove:
                        self.domains[var2].remove(val)
                    # var2 domain is changed, so add to the set
                    queue.append(var2)
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    for i, X_i in enumerate(variables):
        X_i_domain = csp.values[X_i]
        B_i_domain = []
        B_i = ('sum', name, i)
        if i == 0:
            for X_i_val in X_i_domain:
                B_i_domain.append((0, X_i_val))
            csp.add_variable(B_i, B_i_domain)
        else:
            for X_i_val in X_i_domain:
                for B_i_minus_1_val in B_i_minus_1_domain:
                    if B_i_minus_1_val[1] + X_i_val <= maxSum:
                        B_i_domain.append((B_i_minus_1_val[1], B_i_minus_1_val[1] + X_i_val))
            B_i_domain = list(set(B_i_domain))
            B_i_minus_1 = ('sum', name, i - 1)
            csp.add_variable(B_i, B_i_domain)
            def factor(b_i_minus_1, b_i):
                return b_i_minus_1[1] == b_i[0]
            csp.add_binary_factor(B_i_minus_1, B_i, factor)

        def factor(val, b):
            return b[1] == b[0] + val
        csp.add_binary_factor(X_i, B_i, factor)
        B_i_minus_1_domain = B_i_domain
    csp.add_binary_factor(B_i, result, lambda val, res: res == val[1])
    return result
...
class SchedulingCSPConstructor():
        ...
        for request in self.profile.requests:
            for quarter in self.profile.quarters:
                if len(request.cids) > 0:
                    def factor(cid):
                        if cid is None or len(request.quarters)==0 or quarter in request.quarters:
                            return True
                        return False
                    csp.add_unary_factor((request, quarter), factor)
    ...
    def add_unit_constraints(self, csp):
        ...
        pMinUnits = self.profile.minUnits
        pMaxUnits = self.profile.maxUnits
        for quarter in self.profile.quarters:
            variables = []
            for req in self.profile.requests:
                for cid in req.cids:
                    ...
                    domain = [0] + range(minUnits, maxUnits + 1)
                    var = (cid, quarter)
                    csp.add_variable(var, domain)
                    variables.append(var)
                    def factor(course_id, units):
                        if (course_id != cid):
                            return units == 0
                        return units <= maxUnits and units >= minUnits
                    csp.add_binary_factor((req, quarter), (cid, quarter), factor)
            totalUnits = get_sum_variable(csp, quarter, variables, self.profile.maxUnits)
            print(totalUnits)
            def sum_factor(sum):
                return sum <= pMaxUnits and sum >= pMinUnits
            csp.add_unary_factor(totalUnits, sum_factor)
        # END_YOUR_CODE
